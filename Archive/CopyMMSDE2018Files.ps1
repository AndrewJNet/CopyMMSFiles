...
$web = Invoke-WebRequest 'https://mmsde2018.sched.com/login' -SessionVariable mms
...
$mmsHome = Invoke-WebRequest 'https://mmsde2018.sched.com/' -WebSession $mms
...
$web = Invoke-WebRequest 'https://mmsde2018.sched.com/login' -WebSession $mms -Method POST -Body $form.Fields
if(-Not ($web.InputFields.FindByName("login"))) {
    ForEach ($Date in $MMSDates) {
        "Checking day '{0}' for downloads" -f $Date

        $sched = Invoke-WebRequest -Uri $("https://mmsde2018.sched.org/" + $Date + "/list/descriptions") -WebSession $mms
        $links = $sched.Links
